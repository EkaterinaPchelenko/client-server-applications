...
def main():
    ...
    dirictory_path = os.path.dirname(os.path.relpath(__file__))
    config.read(f"{dirictory_path}/{'server.ini'}")
    ...
    def server_config():
        ...
        config_window.db_path.insert(config['SETTINGS']['Database_file'])
        config_window.db_path.insert(config['SETTINGS']['Default_port'])
        config_window.db_path.insert(config['SETTINGS']['Listen_Address'])
    ...
    def save_server_config():
        ...
        config['SETTINGS']['Database_file'] = config_window.db_path.text()
        try:
            ...
            if 1023 < port < 65536:
                ...
                with open('server.ini', 'w') as conf:
                    config.write(conf)
                    message.information(
                        config_window, 'ОК', 'Настройки успешно сохранены')
            else:
                message.warning(
                    config_window,
                    'Ошибка',
                    'Порт должен быть в диапазоне от 1024 до 65536'
                )
    timer = QTimer()
    timer.timeout.connect(update_list)
    timer.start(1000)

    main_window.refresh_button.triggered.connect(update_list)
